%% -------------------------------------------------------------------
%%
%% lasp_orset_delta: A convergent, replicated, state based observe remove set
%%
...
-export_type([orset_delta/0, binary_orset_delta/0, orset_delta_op/0]).
-opaque orset_delta() :: orddict:orddict().

%% A binary that from_binary/1 will operate on.
-type binary_orset_delta() :: binary().

-type orset_delta_op() :: {add, member()} | {remove, member()} |
                    {add_all, [member()]} | {remove_all, [member()]} |
                    {update, [orset_delta_op()]} | {add_by_token, term(), member()}.

-type actor() :: riak_dt:actor().
-type member() :: term().

-spec new() -> orset_delta().
new() ->
    orddict:new().

-spec value(orset_delta()) -> [member()].
value(ORDict0) ->
...
-spec value(any(), orset_delta()) -> [member()] | orddict:orddict().
...
-spec update(orset_delta_op(), actor(), orset_delta(), riak_dt:context()) ->
                    {ok, orset_delta()} | {error, {precondition, {not_present, member()}}}.
update(Op, Actor, ORDict, _Ctx) ->
    update(Op, Actor, ORDict).

-spec parent_clock(riak_dt_vclock:vclock(), orset_delta()) -> orset_delta().
parent_clock(_Clock, ORSet) ->
    ORSet.

-spec merge(orset_delta(), orset_delta()) -> orset_delta().
...
-spec equal(orset_delta(), orset_delta()) -> boolean().
...
%% state to the client.
-spec precondition_context(orset_delta()) -> orset_delta().
...
-spec stats(orset_delta()) -> [{atom(), number()}].
stats(ORSet) ->
...
-spec stat(atom(), orset_delta()) -> number() | undefined.
...
-spec to_binary(orset_delta()) -> binary_orset_delta().
to_binary(ORSet) ->
    <<?TAG:8/integer, ?V1_VERS:8/integer, (riak_dt:to_binary(ORSet))/binary>>.

-spec to_binary(Vers :: pos_integer(), orset_delta()) -> {ok, binary_orset_delta()} | ?UNSUPPORTED_VERSION.
to_binary(1, Set) ->
    {ok, to_binary(Set)};
to_binary(Vers, _Set) ->
    ?UNSUPPORTED_VERSION(Vers).

-spec from_binary(binary_orset_delta()) -> {ok, orset_delta()} | ?UNSUPPORTED_VERSION | ?INVALID_BINARY.
...
-spec to_version(pos_integer(), orset_delta()) -> orset_delta().
to_version(_Version, Set) ->
    Set.

-spec intersect(orset_delta(), orset_delta()) -> orset_delta().
...
-spec map(fun(), orset_delta()) -> orset_delta().
map(Function, V) ->
...
-spec filter(fun((_) -> boolean()), orset_delta()) -> orset_delta().
filter(Function, V) ->
