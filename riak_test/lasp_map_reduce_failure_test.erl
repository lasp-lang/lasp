...
-module(lasp_map_reduce_failure_test).
...
jobtracker(Supervisor, MapTasks, ReduceTasks, Inputs, Outputs) ->
    case MapTasks of
        [MapTask|MT] ->
            ...
            lasp:spawn_mon(Supervisor, Module2, ReduceFun, [MapOut, [], Output]),
        ...
        {'DOWN', Ref, process, _, noproc} ->
            case dict:find(Ref, Dict) of
                {ok, {Module, Function, Args}} ->
                    lasp:spawn_mon(self(), Module, Function, Args);
                error ->
                    supervisor(Dict)
            end;
