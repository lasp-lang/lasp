...
init([]) ->
    lager:info("Group rank calculator server initialized."),
...
handle_info(log, #state{}=State) ->
    ...
    lager:info("Number of Groups: ~p", [sets:size(SetGroupIdToUsers)]),

    %% Print a set.
    {ok, SetGroupSizeAndDivider} = lasp:query(?SET_GROUP_SIZE_AND_DIVIDER),

    lager:info("SetGroupSizeAndDivider: ~p", [SetGroupSizeAndDivider]),
...
handle_info(
    ...
    case SetGroupAndItsRank == ExpectedResult andalso
