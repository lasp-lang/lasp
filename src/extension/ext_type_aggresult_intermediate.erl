-module(ext_type_aggresult_intermediate).
...
-export_type([
    ext_type_aggresult_intermediate/0,
    ext_type_aggresult_intermediate_op/0]).

-opaque ext_type_aggresult_intermediate() :: {?TYPE, payload()}.
-type payload() ::
    {
        ...
        ext_type_orset_base:ext_type_orset_base()} |
    {
        ...
        ext_type_orset_base:ext_type_orset_base()}.
-type element() :: term().
-type ext_type_aggresult_intermediate_op() :: no_op.

-spec new() -> ext_type_aggresult_intermediate().
new() ->
    erlang:error(not_implemented).

-spec new([term()]) -> ext_type_aggresult_intermediate().
new([_NodeId]) ->
...
-spec mutate(
    ext_type_aggresult_intermediate_op(), type:id(), ext_type_aggresult_intermediate()) ->
    {ok, ext_type_aggresult_intermediate()}.
mutate(no_op, _Actor, {?TYPE, _AggResultORSetPayload}=CRDT) ->
    {ok, CRDT}.

-spec query(ext_type_aggresult_intermediate()) -> sets:set(element()).
...
-spec equal(ext_type_aggresult_intermediate(), ext_type_aggresult_intermediate()) -> boolean().
equal(
...
-spec delta_mutate(
    ext_type_aggresult_intermediate_op(), type:id(), ext_type_aggresult_intermediate()) ->
    {ok, ext_type_aggresult_intermediate()}.
delta_mutate(_Op, _Actor, {?TYPE, _AggResultORSetPayload}=CRDT) ->
    {ok, CRDT}.

-spec merge(ext_type_aggresult_intermediate(), ext_type_aggresult_intermediate()) ->
    ext_type_aggresult_intermediate().
merge({?TYPE, AggResultORSetPayload}, {?TYPE, AggResultORSetPayload}) ->
    {?TYPE, AggResultORSetPayload};
merge(
    {?TYPE, {intermediate, ORSetBaseL}=_AggResultORSetPayloadL},
    {?TYPE, {intermediate, ORSetBaseR}=_AggResultORSetPayloadR}) ->
    NewORSetBase = do(join, [intermediate, ORSetBaseL, ORSetBaseR]),
    {?TYPE, {intermediate, NewORSetBase}};
merge(
    {?TYPE, {intermediate, AllPathInfoList, ORSetBaseL}=_AggResultORSetPayloadL},
    {?TYPE, {intermediate, AllPathInfoList, ORSetBaseR}=_AggResultORSetPayloadR}) ->
    NewORSetBase = do(join, [intermediate, ORSetBaseL, ORSetBaseR]),
    {?TYPE, {intermediate, AllPathInfoList, NewORSetBase}}.

-spec is_bottom(ext_type_aggresult_intermediate()) -> boolean().
is_bottom({?TYPE, {intermediate, ORSetBase}=_AggResultORSetPayload}=_CRDT) ->
    ORSetBase == do(new, [undefined]);
is_bottom(
    {?TYPE, {intermediate, AllPathInfoList, ORSetBase}=_AggResultORSetPayload}=_CRDT) ->
    ORSetBase == do(new, [AllPathInfoList]);
is_bottom(_CRDT) ->
    false.

-spec is_inflation(
    ext_type_aggresult_intermediate(), ext_type_aggresult_intermediate()) -> boolean().
is_inflation(
    {?TYPE, {intermediate, ORSetBaseL}=_AggResultORSetPayloadL},
    {?TYPE, {intermediate, ORSetBaseR}=_AggResultORSetPayloadR}) ->
...
-spec query_ext(
    {ext_type_cover:ext_subset_in_cover(), ext_type_aggresult_intermediate()}) ->
...
query_ext_consistent(
    {PrevSubset, PrevCDS, {?TYPE, {intermediate, ORSetBase}}}) ->
