...
-spec new(ext_type_path:ext_path_info_list()) -> ext_type_orset_base_v1().
...
-spec equal(ext_type_orset_base_v1(), ext_type_orset_base_v1()) -> boolean().
equal(
    {EventHistoryAllL, EventHistorySurvivedL, CoverL, DataStoreL}=_ORSetBaseV1L,
    {EventHistoryAllR, EventHistorySurvivedR, CoverR, DataStoreR}=_ORSetBaseV1R) ->
...
-spec insert(ext_type_event_history:ext_event_history(), element(), ext_type_orset_base_v1()) ->
    ext_type_orset_base_v1().
insert(EventHistory, Elem, {[], [], [], []}=_ORSetBaseV1) ->
    NewProvenance =
        ext_type_provenance:new_provenance(ext_type_provenance:new_dot(EventHistory)),
    NewSubsetInCover = ext_type_cover:new_subset_in_cover(EventHistory),
    {
...
-spec read(ext_type_cover:ext_subset_in_cover(), ext_type_orset_base_v1()) ->
    {ext_type_cover:ext_subset_in_cover(), sets:set()}.
read(
    PrevSubset,
    {_EventHistoryAll, _EventHistorySurvived, Cover, DataStore}=_ORSetBaseV1) ->
    case DataStore of
        [] ->
...
-spec join(ext_type_orset_base:ext_node_type(), ext_type_orset_base_v1(), ext_type_orset_base_v1()) ->
    ext_type_orset_base_v1().
join(_, {[], [], [], []}, ORSetBaseV1) ->
    ORSetBaseV1;
join(_, ORSetBaseV1, {[], [], [], []}) ->
    ORSetBaseV1;
join(
    _NodeType, ORSetBaseV1, ORSetBaseV1) ->
    ORSetBaseV1;
join(
    input,
    {
        ...
        DataStoreL}=_ORSetBaseV1L,
    {
        EventHistoryAllR,
        EventHistorySurvivedR,
        [SingleSubsetR]=_CoverR,
        DataStoreR}=_ORSetBaseV1R) ->
...
join(
    ...
    {JoinedEventHistoryAll, JoinedEventHistorySurvived, JoinedCover, JoinedDataStore}.

-spec is_inflation(ext_type_orset_base_v1(), ext_type_orset_base_v1()) -> boolean().
is_inflation(
    {EventHistoryAllL, EventHistorySurvivedL, CoverL, DataStoreL}=_ORSetBaseV1L,
    {EventHistoryAllR, EventHistorySurvivedR, CoverR, DataStoreR}=_ORSetBaseV1R) ->
...
-spec is_strict_inflation(ext_type_orset_base_v1(), ext_type_orset_base_v1()) -> boolean().
...
-spec map(
    ...
    ext_type_orset_base_v1()) -> ext_type_orset_base_v1().
map({NodeId, _ReplicaId}=_Actor,
    Function,
    _AllPathInfoList,
    PathInfo,
    {EventHistoryAll, EventHistorySurvived, Cover, DataStore}=_ORSetBaseV1) ->
    NewEventHistoryAll =
        ext_type_event_history_set:append_cur_node(NodeId, PathInfo, EventHistoryAll),
    NewEventHistorySurvived =
        ext_type_event_history_set:append_cur_node(NodeId, PathInfo, EventHistorySurvived),
    {NewCover, NewDataStore} =
        ordsets:fold(
            fun(SubsetInCover, {AccInNewCover, AccInNewDataStore}) ->
                ...
                NewSubsetInCover = ext_type_cover:append_cur_node(NodeId, PathInfo, SubsetInCover),
                {
...
product(
    ...
    ProductEventHistoryAll =
        ext_type_event_history_set:append_cur_node(
            ...
            PathInfo,
            ext_type_event_history_set:union_event_history_set(
                EventHistoryAllL, EventHistoryAllR)),
    ...
    ProductEventHistorySurvived =
        ext_type_event_history_set:append_cur_node(NodeId, PathInfo, ProductEventHistorySurvived0),
    ...
    {NewCover, NewDataStore, NewGroupEventHistorySet} =
        ordsets:fold(
            fun(Subset,
                ...
                NewSubset = ext_type_event_history_set:append_cur_node(NodeId, PathInfo, Subset),
                CDS =
...
-spec next_event_history(
    ...
    ext_node_id(),
    ext_replica_id(),
    ext_type_orset_base_v1()) -> ext_type_event_history:ext_event_history().
next_event_history(
    EventType,
    NodeId,
    ReplicaId,
    {EventHistoryAll, _EventHistorySurvived, _Cover, _DataStore}=_ORSetBaseV1L) ->
