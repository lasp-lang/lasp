...
-opaque ext_type_orset_base_v3() ::
    {ext_event_history_all(), ext_event_removed(), ext_data_store()}.

-spec new(ext_type_path:ext_path_info_list()) -> ext_type_orset_base_v3().
new(_AllPathInfoList) ->
    {ext_type_event_history_set:new_event_history_set(), ordsets:new(), orddict:new()}.

-spec equal(ext_type_orset_base_v3(), ext_type_orset_base_v3()) -> boolean().
equal(
    {EventHistoryAllL, EventRemovedL, DataStoreL}=_ORSetBaseV3L,
    {EventHistoryAllR, EventRemovedR, DataStoreR}=_ORSetBaseV3R) ->
    EventHistoryAllL == EventHistoryAllR andalso
        EventRemovedL == EventRemovedR andalso
        DataStoreL == DataStoreR.

-spec insert(ext_type_event_history:ext_event_history(), element(), ext_type_orset_base_v3()) ->
    ext_type_orset_base_v3().
insert(EventHistory, Elem, {[], [], []}=_ORSetBaseV3) ->
    NewProvenance =
    ...
    NewSubsetUnknownInCover = ext_type_cover:new_subset_in_cover(),
    {
        ...
        orddict:store(
            ...
            orddict:new())};
insert(EventHistory, Elem, {EventHistoryAll, EventRemoved, DataStore}=ORSetBaseV3) ->
    CanBeSkipped =
...
-spec join(
    ext_type_orset_base:ext_node_type(), ext_type_orset_base_v3(), ext_type_orset_base_v3()) ->
    ext_type_orset_base_v3().
join(_, {[], [], []}, ORSetBaseV3) ->
    ORSetBaseV3;
join(_, ORSetBaseV3, {[], [], []}) ->
    ORSetBaseV3;
join(
    _NodeType, ORSetBaseV3, ORSetBaseV3) ->
    ORSetBaseV3;
join(
    input,
    {
        EventHistoryAllL,
        EventRemovedL,
        [{SingleSubsetUnknownL, ProvenanceStoreL}]=_DataStoreL}=_ORSetBaseV3L,
    {
        EventHistoryAllR,
        EventRemovedR,
        [{SingleSubsetUnknownR, ProvenanceStoreR}]=_DataStoreR}=_ORSetBaseV3R) ->
...
-spec is_inflation(ext_type_orset_base_v3(), ext_type_orset_base_v3()) -> boolean().
is_inflation(
    {EventHistoryAllL, EventRemovedL, DataStoreL}=_ORSetBaseV3L,
    {EventHistoryAllR, EventRemovedR, DataStoreR}=_ORSetBaseV3R) ->
...
-spec is_strict_inflation(ext_type_orset_base_v3(), ext_type_orset_base_v3()) -> boolean().
...
-spec map(
    ...
    ext_type_orset_base_v3()) -> ext_type_orset_base_v3().
map({NodeId, _ReplicaId}=_Actor,
    Function,
    PathInfo,
    {EventHistoryAll, EventRemoved, DataStore}=_ORSetBaseV3) ->
    NewEventHistoryAll =
...
-spec next_event_history(
    ...
    ext_node_id(),
    ext_replica_id(),
    ext_type_orset_base_v3()) -> ext_type_event_history:ext_event_history().
next_event_history(
    EventType,
    NodeId,
    ReplicaId,
    {EventHistoryAll, _EventRemoved, _DataStore}=_ORSetBaseV3L) ->
