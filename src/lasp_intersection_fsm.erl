...
start_link(ReqId, From, Left, Right, Intersection) ->
    gen_fsm:start_link(?MODULE, [ReqId, From, Left, Right, Intersection], []).
...
init([ReqId, From, Left, Right, Intersection]) ->
...
execute(timeout, #state{preflist=Preflist,
    ...
    lasp_vnode:intersection(Preflist, {ReqId, Coordinator}, Left, Right, Intersection),
