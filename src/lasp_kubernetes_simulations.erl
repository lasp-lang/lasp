...
delete_replicasets(Run) ->
    ...
    APIServer = os:getenv("APISERVER"),
    PodsUrl = APIServer ++ "/apis/extensions/v1beta1/namespaces/default/replicasets?labelSelector=run%3D" ++ Run ++ ",evaluation-identifer%3D" ++ atom_to_list(EvaluationIdentifier) ++ ",evaluation-timestamp%3D" ++ integer_to_list(EvaluationTimestamp),
...
delete_pods(Run) ->
    ...
    APIServer = os:getenv("APISERVER"),
    PodsUrl = APIServer ++ "/api/v1/pods?labelSelector=run%3D" ++ Run ++ ",evaluation-identifer%3D" ++ atom_to_list(EvaluationIdentifier) ++ ",evaluation-timestamp%3D" ++ integer_to_list(EvaluationTimestamp),
