...
start_link(ReqId, From, Id) ->
    gen_fsm:start_link(?MODULE, [ReqId, From, Id], []).

%% @doc Next id for a variable.
next(Id) ->
    ReqId = lasp:mk_reqid(),
    _ = lasp_next_fsm_sup:start_child([ReqId, self(), Id]),
    {ok, ReqId}.
...
execute(timeout, #state{preflist=Preflist,
    ...
    lasp_vnode:next(Preflist, {ReqId, Coordinator}, Id),
