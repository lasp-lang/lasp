...
union({state_oorset_ps, {delta, Delta}}, {state_oorset_ps, _ORSet}) ->
    lager:warning("union incremental (left)"),
    {state_oorset_ps, {delta, Delta}};
union({state_oorset_ps, _ORSet}, {state_oorset_ps, {delta, Delta}}) ->
    lager:warning("union incremental (right)"),
    {state_oorset_ps, {delta, Delta}};
union({state_oorset_ps, ORSetL}, {state_oorset_ps, ORSetR}) ->
    lager:warning("union"),
    state_oorset_ps:merge({state_oorset_ps, ORSetL}, {state_oorset_ps, ORSetR});
union(LValue, RValue) ->
    state_orset:merge(LValue, RValue).

product({state_oorset_ps, {delta, Delta}}, {state_oorset_ps, ORSet}) ->
    lager:warning("product incremental (left)"),
    {state_oorset_ps, ProductDelta} =
        product({state_oorset_ps, Delta}, {state_oorset_ps, ORSet}),
    {state_oorset_ps, {delta, ProductDelta}};
product({state_oorset_ps, ORSet}, {state_oorset_ps, {delta, Delta}}) ->
    lager:warning("product incremental (right)"),
    {state_oorset_ps, ProductDelta} =
        product({state_oorset_ps, ORSet}, {state_oorset_ps, Delta}),
    {state_oorset_ps, {delta, ProductDelta}};
...
intersect({state_oorset_ps, {delta, Delta}}, {state_oorset_ps, ORSet}) ->
    lager:warning("intersect incremental (left)"),
    {state_oorset_ps, ProductDelta} =
        intersect({state_oorset_ps, Delta}, {state_oorset_ps, ORSet}),
    {state_oorset_ps, {delta, ProductDelta}};
intersect({state_oorset_ps, ORSet}, {state_oorset_ps, {delta, Delta}}) ->
    lager:warning("intersect incremental (right)"),
    {state_oorset_ps, ProductDelta} =
        intersect({state_oorset_ps, ORSet}, {state_oorset_ps, Delta}),
    {state_oorset_ps, {delta, ProductDelta}};
intersect({state_oorset_ps, {{ElemDataStoreL, EventDataStoreL},
                             FilteredOutEventsL, AllEventsAnyL}=_ORSetL},
          {state_oorset_ps, {{ElemDataStoreR, EventDataStoreR},
                             FilteredOutEventsR, AllEventsAnyR}=_ORSetR}) ->
    lager:warning("intersect"),
    %% Remove the removed events of the other set first (optimisation)
    ValidEventsL = ordsets:union(
                     ordsets:from_list(orddict:fetch_keys(EventDataStoreL)),
                     FilteredOutEventsL),
...
map(Function, {state_oorset_ps, {delta, Payload}}) ->
    lager:warning("map incremental"),
    {state_oorset_ps, MapDelta} = map(Function, {state_oorset_ps, Payload}),
    {state_oorset_ps, {delta, MapDelta}};
...
filter(Function, {state_oorset_ps, {delta, Payload}}) ->
    lager:warning("filter incremental"),
    {state_oorset_ps, FilterDelta} = filter(Function, {state_oorset_ps, Payload}),
    {state_oorset_ps, {delta, FilterDelta}};
filter(Function, {state_oorset_ps, {{ElemDataStore, EventDataStore},
                                    FilteredOutEvents, AllEvents}}) ->
    lager:warning("filter"),
